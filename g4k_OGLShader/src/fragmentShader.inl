...
"float r(vec3 m)"
"{"
"if(t(124.)>.9&&t(160.)<.9||t(232.)>.9&&t(240.)<.9||t(256.)>.9&&t(272.)<.9||t(288.)>.9&&t(304.)<.9||t(336.)>.9&&t(380.)<.9)"
"{"
"vec3 s=vec3(6,0,0);"
"float y=6.28/15.+sin(v*.35)*.1,i=1e4;"
"for(float r=0.;r<3.;r+=1.)"
"for(float x=0.;x<15.;x+=1.)"
"{"
"vec3 c=m-vec3(0,-2.+r*2.,0)-f(x*y+v*.65)*s;"
...
"f*=n()*2.-1.;"
"vec3 y=vec3(8.*cos(f),.6,8.*sin(f)),r=normalize(vec3(0,sin(v*.5),0)-y),l=normalize(cross(r,vec3(0,.6,.5)));"
"r=normalize(m.x*l+m.y*normalize(cross(l,r))+1.5*r);"
"f=0.;"
"float t=1e5,z=0.;"
"for(int v=0;v<128;v++)"
"{"
