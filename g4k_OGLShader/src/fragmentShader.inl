...
"return opSubtraction(d1,sdBox(repeat(vec3(0,0,5.*sin(iGlobalTime*.1))+pos*rotate_x(iGlobalTime*.1),vec3(10))));"
"}"
...
"if(an<35.)"
"{"
"vec3 pos=ro+an*uu,nor=calcNormal(pos);"
"float dif=clamp(dot(nor,vec3(.7,1.6,.4)),0.,1.2);"
"dif*=dif;"
"dif*=dif;"
"ww=vec3(.2,.3,.5)*(.5+.5*dot(nor,vec3(0,.8,-.6)))+stripes(mandelboxPosition)*vec3(.9,.8,.6)*dif;"
"ww*=ambientOcclusion(pos,nor);"
"}"
"ww=sqrt(ww);"
"fragColor=vec4(ww,1);"
"}";